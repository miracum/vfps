...
|-----|------|---------|-------------|
| affinity | object | `{}` |  |
| auth.clientCertificate.allowedThumbprints | list | `[]` | SHA-256 thumbprints allowed to call, hex encoded. Empty accepts any certificate chaining to a trusted root, which is only as narrow as that root. |
