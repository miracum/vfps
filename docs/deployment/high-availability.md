...
- **The `appsettings` volume is declared unconditionally.** `deployment.yaml` always lists the
  ...
  `ContainerCreating` with `configmap ... not found`. Note `ci/kitchen-sink-test-values.yaml` _does_
...
- **`/readyz` never fails.** It filters on a `ready` health-check tag that nothing is registered
  ...
  dependency - a database blip would otherwise pull _every_ replica out of service simultaneously,
